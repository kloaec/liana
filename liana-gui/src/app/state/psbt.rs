        ...
        match message {
            ...
            Message::Updated(Ok(_)) => {
                ...
                if let Some(modal) = self.modal.as_mut() {
                    ...
                    // The daemon gave us the status when it listed the psbt, signing here makes it
                    // stale. Not once broadcast: the coins only show the spend after the next poll.
                    if self.tx.status != SpendStatus::Broadcast {
                        self.tx
                            .refresh_status(&self.wallet.main_descriptor, cache.blockheight());
                    }
